...
//	FSMC_NORSRAMInit(&FSMC_NORSRAMInitStructure); 
	...
	/* FSMC写速度设置 */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressSetupTime = 20;//1;   /* 地址建立时间  */
	FSMC_NORSRAMTimingInitStructure.FSMC_AddressHoldTime = 0;	   
	FSMC_NORSRAMTimingInitStructure.FSMC_DataSetupTime = 20;//1;	   /* 数据建立时间  */
...
* Attention		 : None
*******************************************************************************/
void GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)//宽8 高16
{
    uint8_t TempChar;
    do
    {
...
{	
	...
	sprintf(lcd_buff1,"FPS:%d",str);
	GUI_Text( Xpos, Ypos, lcd_buff1, Color, bkColor );
}
...
void GUI_Num(uint16_t Xpos, uint16_t Ypos, int16_t number,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar[3],idx=0;
	  if((number>999)||(number<0))//超出0~999范围
		{
			TempChar[0]=TempChar[1]=TempChar[2]='*';
		}
		else
		{
			TempChar[2] = number%10+'0';//个位
			TempChar[1] = number/10%10+'0';//十位
			TempChar[0] = number/10/10%10+'0';//百位	
		}
		
		
    do
    {
        PutChar( Xpos, Ypos, TempChar[idx], Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8;
        } 
